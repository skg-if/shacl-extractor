...
class TestHelperFunctionsWithFiles(TempDirTestCase):
    def test_literal_prefix_resolution_in_shapes(self):
        ttl_file = Path(self.temp_dir) / "literal-prefix.ttl"
...
''')
    ...
    def test_union_range_generates_sh_or(self):
        ttl_file = Path(self.temp_dir) / "union-range.ttl"
        with open(ttl_file, 'w', encoding='utf-8') as f:
            f.write('''
@prefix owl: <http://www.w3.org/2002/07/owl#> .
@prefix dc: <http://purl.org/dc/elements/1.1/> .
...
''', format='turtle')
    ...
    def test_unqualified_target_name(self):
        ttl_file = Path(self.temp_dir) / "unqualified.ttl"
        with open(ttl_file, 'w', encoding='utf-8') as f:
            f.write('''
@prefix owl: <http://www.w3.org/2002/07/owl#> .
@prefix dc: <http://purl.org/dc/elements/1.1/> .
